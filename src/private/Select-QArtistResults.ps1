...
.DESCRIPTION
    Select-QArtistResults applies a configurable scoring function to a list
...
#>
function Select-QArtistResults {
    [CmdletBinding()]
    param(
    ...
    process {
        # Normalize input (defensive: Convert-TextNormalized requires non-empty string)
        ...
        if ($InputArtist -and ($InputArtist.ToString().Trim().Length -gt 0)) {
            try { $normInput = Convert-TextNormalized -Text $InputArtist } catch { $normInput = '' }
        } else { $normInput = '' }
        $normAlbum = ''
        if ($AlbumName -and ($AlbumName.ToString().Trim().Length -gt 0)) {
            try { $normAlbum = Convert-TextNormalized -Text $AlbumName } catch { $normAlbum = '' }
        }
        ...
        foreach ($c in $Candidates) {
            ...
            if (-not $cName) { $cName = '' }
            if ($cName -and ($cName.Trim().Length -gt 0)) {
                try { $normC = Convert-TextNormalized -Text $cName } catch { $normC = '' }
            } else {
                $normC = ''
            }
            ...
            $albumBonus = 0.0
            if ($normAlbum) {
                # Heuristic: check whether the album name (spaces replaced by dashes) appears in the candidate link
                try {
                    $albumEsc = ($AlbumName -as [string]) -replace '\s+','-'
                    $albumPattern = [regex]::Escape($albumEsc)
                    if ($c.Link -and ($c.Link -match $albumPattern)) { $albumBonus = 0.25 }
                } catch {
